    ...
    while True:
        try:
            ...
            result = model.decode(
                ...
                top_p=args.top_p,
                use_cache=not args.disable_cache  # Use cache unless disabled
            )
            
            # Handle the return value based on cache usage
            if args.disable_cache:
                generated_text = result  # Just the string
            else:
                generated_text, _ = result  # Unpack tuple (text, cache)
...
Example usage:
  ...
  # Disable KV cache for memory-constrained environments
  python generate.py --config config.json --checkpoint model.pt \\
    --tokenizer-path tokenizer --prompt "Hello" --disable-cache
        """
    )
    ...
    try:
        ...
        cache_status = "disabled" if args.disable_cache else "enabled"
        print(f"KV cache: {cache_status}")
